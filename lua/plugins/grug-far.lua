...
local function search_on_directory(opts)
  ...
  if opts.select_word then
    ...
    require("grug-far").grug_far({ prefills = opts, transient = true })
  else
...
return {
  {
    ...
    -- stylua: ignore
    keys = {
      { "<leader>F", function() require('grug-far').grug_far({transient = true}) end, desc = "Search & Replace in Project", },
      { "<leader>fw", function() require('grug-far').grug_far({ prefills = { search = vim.fn.expand("<cword>") }, transient = true }) end, desc = "Search & Replace current word in Project" },
